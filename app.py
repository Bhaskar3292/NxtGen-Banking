...
def editcustomer(cust_ssn_id):
    ...
    if request.method == "POST":
        ...
        update_values = {}
        print(customerssn)
        update_values['SSN'] = cust_ssn_id  # Ensure2 SSN is added here
...
def deposit(acc_id=None):
    ...
    if acc_id is None:  # Redirect to the account list page if acc_id is not provided
        flash("No account selected. Please select an account to deposit.", "danger")
        return redirect(url_for('viewaccount'))
    
    cursor = connection.cursor()
    
    if request.method == "GET":
        cursor.execute("SELECT * FROM ACCOUNTS WHERE ACCOUNTNUMBER = :1", [acc_id])
        account = cursor.fetchone()
        if account:
            data = {
                'ACCOUNTNUMBER': account[0],
        ...
        if not amount or not amount.isdigit():
            ...
            return redirect(url_for('deposit', acc_id=acc_id))

        amount = float(amount)
        print(acc_id)
        cursor.execute("SELECT BALANCE FROM ACCOUNTS WHERE ACCOUNTNUMBER = :1", [acc_id])
        account = cursor.fetchone()
        if account:
            new_balance = float(account[0]) + amount
            cursor.execute("UPDATE ACCOUNTS SET BALANCE = :1 WHERE ACCOUNTNUMBER = :2", [new_balance, acc_id])
            connection.commit()
            flash(f"Successfully deposited {amount} into account {acc_id}.", "success")
            return redirect(url_for('viewaccount'))
        else:
            flash("Account not found.", "danger")
            return redirect(url_for('viewaccount'))
...
def withdraw(acc_id=None):
    ...
    if request.method == "GET":
        ...
        account = cursor.fetchone()
        if account:
            new_balance = amount - float(account[0])
            cursor.execute("UPDATE ACCOUNTS SET BALANCE = :1 WHERE ACCOUNTNUMBER = :2", [new_balance, acc_id])
            connection.commit()
            flash(f"Successfully deposited {amount} into account {acc_id}.", "success")
            return redirect(url_for('viewaccount'))
...
def transfer(cust_id=None):
    ...
    if session['usert'] in ["customer", "cashier"]:
        if not cust_id:
            return redirect(url_for('viewaccount'))
        ...
        if request.method == 'POST':
            ...
            if not amount or not amount.isdigit():
                ...
                return redirect(url_for('transfer', cust_id=cust_id))
            ...
            if src_type != trg_type:
                try:
                    ...
                    src_query = """
                        ...
                        FROM ACCOUNTS 
                        WHERE CUSTOMER_SSN = :cust_id AND ACCOUNTTYPE = :src_type
                    """
                    print(cust_id)
                    trg_query = """
                        SELECT ACCOUNTNUMBER, BALANCE 
                        FROM ACCOUNTS 
                        WHERE CUSTOMER_SSN = :cust_id AND ACCOUNTTYPE = :trg_type
                    """
                    src_data = cursor.execute(src_query, {"cust_id": cust_id, "src_type": src_type}).fetchone()
                    trg_data = cursor.execute(trg_query, {"cust_id": cust_id, "trg_type": trg_type}).fetchone()
                    print(src_data)
                    print(trg_data)
                    if src_data is None:
                    ...
                    src_account, src_balance = src_data
                    trg_account, trg_balance = trg_data
                    ...
                    if src_balance >= amount:
                        ...
                        cursor.execute(update_src_query, {"balance": src_balance, "account_number": src_account})
                        cursor.execute(update_trg_query, {"balance": trg_balance, "account_number": trg_account})
                ...
                flash("Source and target account types cannot be the same.", "danger")

    return render_template('transfer.html', cust_id=cust_id)
